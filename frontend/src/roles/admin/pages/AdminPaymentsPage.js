...
export default function AdminPaymentsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setStats(statsRes.data.data || { totalRevenue: 0, collected: 0, pending: 0, collectedPercentage: 0 });
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const printReceipt = (inv) => {
    ...
    win.document.write(`
      ...
      ${inv.discountAmount > 0 ? `<div class="row"><span>Discount:</span> <span>-$${inv.discountAmount.toLocaleString()}</span></div>` : ""}
      <div class="row"><span>Paid to Date:</span> <span>$${(inv.paidAmount || 0).toLocaleString()}</span></div>
      <div class="total-row"><span>Remaining Balance:</span> <span>$${(netAmount - (inv.paidAmount || 0)).toLocaleString()}</span></div>
      <div class="row"><strong>Status:</strong> <span class="badge" style="background:${inv.status === "paid" ? "#d1fae5;color:#065f46" : "#fef3c7;color:#92400e"}">${inv.status.toUpperCase()}</span></div>
      <div class="divider"></div>
  ...
  return (
    <div className="container-fluid p-3 p-md-4 px-md-5">
      ...
      <div className="row g-3 mb-4">
        {[
          { label: "Total Revenue", value: `$${(stats.totalRevenue || 0).toLocaleString()}`, icon: <CheckCircle size={22} />, color: "primary", subtitle: `${stats.collectedPercentage || 0}% Collected` },
          { label: "Collected Cash", value: `$${(stats.collected || 0).toLocaleString()}`, icon: <CreditCard size={22} />, color: "success" },
          { label: "Pending Balance", value: `$${(stats.pending || 0).toLocaleString()}`, icon: <Clock size={22} />, color: "warning" },
                  ...
                  return (
                    <tr key={inv._id}>
                      ...
                      <td className="py-3">
                        ...
                        <div className="text-muted" style={{ fontSize: "0.78em" }}>Total: ${netTotal.toLocaleString()}</div>
                      </td>
                      <td className="py-3" style={{ minWidth: "150px" }}>
                        <div className="d-flex justify-content-between small mb-1">
                          <span>${(inv.paidAmount || 0).toLocaleString()} paid</span>
                          <span className="text-muted">{progress}%</span>
                        </div>
                        <div className="progress" style={{ height: "6px" }}>
                          <div className={`progress-bar bg-${progress === 100 ? "success" : "primary"}`} style={{ width: `${progress}%` }} />
                        </div>
                        {remaining > 0 && <div className="text-danger mt-1" style={{ fontSize: "0.78em" }}>Left: ${remaining.toLocaleString()}</div>}
                      </td>
                        ...
                        <div className="d-flex gap-2 justify-content-end">
                          ...
                          {inv.status !== "paid" && (
                            <button
                              ...
                              onClick={() => { setShowPayModal(inv); setPayData({ amount: remaining, method: "cash" }); }}
                            >
                ...
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <select className="form-select" required value={formData.studentId} onChange={e => setFormData({ ...formData, studentId: e.target.value })}>
                      <option value="">Select student for billing…</option>
                      {students.map(s => (
            ...
            <div className="modal-content border-0 shadow">
              ...
              <form onSubmit={handleRecordPayment}>
                <div className="modal-body text-center py-4">
                  <div className="mb-3 mx-auto bg-success-subtle text-success rounded-circle d-flex align-items-center justify-content-center" style={{ width: "60px", height: "60px" }}>
                    <CreditCard size={30} />
                  </div>
                      ...
                      <div className="btn-group w-100" role="group">
                        {["cash", "card", "transfer"].map(m => (
                          <React.Fragment key={m}>
                            <input type="radio" className="btn-check" name="method" id={`method-${m}`}
                              checked={payData.method === m} onChange={() => setPayData({ ...payData, method: m })} />
